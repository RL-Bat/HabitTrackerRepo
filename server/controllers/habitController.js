...
habitController.editRunningTotal = async (req, res, next) => {
  try {
    ...
    const _id = newObj._id;
    const user = await models.HabitsData.findOne({ _id });
    const cardIndex = user.habitCards.findIndex((obj => obj.cardId == newObj.cardId));
    // converts to number because of postman
    user.habitCards[cardIndex].runningTotal = Number(newObj.updated) + user.habitCards[cardIndex].runningTotal 
